#!/usr/bin/env python
"""
Validate noise simulations for LAT by comparing measured power spectra with theory.
"""
...
def get_output_folder(yaml_path, base_output_dir):
    ...
    # For instr_params_lat_channels.yaml -> lat_channels
...
def get_lat_files(ch_name, resource_dir):
    """Return relhits and binary mask paths based on channel name."""
    if 'LF' in ch_name:
        band = 'lf'
    elif 'MF' in ch_name:
        band = 'mf'
    ...
    relhits = os.path.join(resource_dir, f'so_lat_{band}_relhits_2048.fits')
    binary = os.path.join(resource_dir, f'so_lat_{band}_bin_2048.fits')
...
def get_theory_Nl_ip(Nl_white_I, Nl_white_P, params, lmax):
    ...
    Nl_I = np.zeros(lmax + 1)
    Nl_P = np.zeros(lmax + 1)
    
    # Avoid division by zero at ell=0,1 if needed, or just slice
    # Model: N(l) = N_white * (1 + (l/l_knee)^alpha)
    
    # Intensity
    ell_knee_i = params['ell_knee_i']
    alpha_knee_i = params['alpha_knee_i']
    factor_i = np.ones_like(ells)
    factor_i[2:] = (1. + (ells[2:] / ell_knee_i)**alpha_knee_i)
    Nl_I = Nl_white_I * factor_i
    
    # Polarization 
    ell_knee_p = params['ell_knee_p']
    alpha_knee_p = params['alpha_knee_p']
    factor_p = np.ones_like(ells)
    factor_p[2:] = (1. + (ells[2:] / ell_knee_p)**alpha_knee_p)
...
def main():
    ...
    for ch_name in channels:
        ...
        nside = ch_params.get('nside', 2048) # Default to 2048 for LAT
        
        # Get masking files
        relhits_file, binary_file = get_lat_files(ch_name, RESOURCE_DIR)
        ...
        # LAT hits masks are well defined, cut below threshold
        weight[relhits >= 1e-2] = relhits[relhits >= 1e-2]
        
        fsky = st.fsky(weight)
        
        binary_mask = hp.read_map(binary_file)
        # Apodize mask
        ...
        if not os.path.exists(noise_file_test):
             ...
             # specific check for LAT naming if typical convention fails?
             # But we assume run_generate_noise_lat uses standard convention.
             continue

        noise_map = hp.read_map(noise_file_test, field=None)  # Read all 3 fields (I, Q, U)
        ...
        for i, (ax, title) in enumerate(zip(axes, titles)):
            ...
            # Use mask for dynamic range calculation
        ...
        bins = nmt.NmtBin.from_lmax_linear(LMAX, 30) # Coarser bins for LAT high L
        ...
        for sim_idx in tqdm(range(NSIMS_FOR_AVG), desc=f"  {ch_name} spectra", ncols=120):
            ...
            # Homogenized noise for NMT
            homo_noise_map = noise_map * np.sqrt(weight)
            
            if sim_idx == 0:
                # Compute coupling matrix once
                # Note: purify_e/b set to False for noise validation usually
            ...
            # For NaMaster, process T and Pol separately or together
            # Here doing T then P
            # Create field just for computing coupled Cls?
            # Creating field is expensive, maybe just use anafast on masked map and decouple?
            # NaMaster: Cl_coupled = M * Cl_true
            # Cl_coupled approx = anafast(map * mask)
            # So we can decouple the anafast result if we don't want to re-create NmtField every sim
        ...
        # =====================================================================
        # Part 3: Estimate White Noise Level and Compute Theory
        # =====================================================================
        print(f"  Estimating white noise level and computing theory...")
        
        # Region for white noise estimation (ell 3000 to 4500)
        idx_min = 3000
        idx_max = 4500
        if idx_max > LMAX: idx_max = LMAX
        
        # Estimate N_white from measured high-ell average
        # TT
        Nl_white_TT_meas = np.mean(cl_avg_TT[idx_min:idx_max])
        # EE/BB (average them for P)
        Nl_white_P_meas = np.mean([np.mean(cl_avg_EE[idx_min:idx_max]), np.mean(cl_avg_BB[idx_min:idx_max])])
        
        # Convert to uK-arcmin for reporting
        ...
        # Compute full theory curves using estimated white noise floors
        ...
        # Params for plot title
        p_str = f"lk_T={ch_params['ell_knee_i']}, a_T={ch_params['alpha_knee_i']} | lk_P={ch_params['ell_knee_p']}, a_P={ch_params['alpha_knee_p']}"
        ...
        plt.suptitle(f'{ch_name} Noise Validation\n{p_str}\nEst Depths: T={uKarcmin_TT:.1f}, P={uKarcmin_P:.1f} uK-min', fontsize=10, y=1.05)
        plt.tight_layout()
        plt.savefig(os.path.join(validation_folder, f'power_spectrum_{ch_name}.png'), dpi=150, bbox_inches='tight')
        plt.close()
